...
function cuentaValida(codigo) {
    let codigoCad = String(codigo); //Convertir el codigo a string
    let numero = parseInt(codigoCad); //convertir el codigo a número

    if (!/^\d{6}$/.test(codigoCad)) { //(Uso de expresión regular) Si el código no contiene 6 dígitos
...
class Banco { //Declaración clase Banco
    ...
    crearCuenta(codigo, saldo = 0) { //Método crearCuenta que recibe el código de la cuenta y el saldo inicial, que por defecto es 0.
        /*Se valida el código de cuenta usando la función cuentaValida anterior. 
        Si el código no es válido, sale del método*/
        let codigoCad = cuentaValida(codigo); 
        if(!codigoCad) { 
            return;
        }

        else if (codigoCad in this.cuentas) { //Se comprueba si el código introducido está ya en cuentas
            console.log("La cuenta ya existe."); //Si está, no se puede crear la cuenta porque ya existe una con ese código
            return;
        }
        this.cuentas[codigoCad] = saldo; //Si el código es válido y no existe ya la cuenta, se crea la cuenta en el objeto cuentas con el saldo inicial
    }
    ...
    actualizarCuenta(codigo, importe) { //Método actualizarCuenta que recibe el codigo de cuenta y el importe (negativo o positivo)
        ...
        else if (!(codigoCad in this.cuentas)) { //Se comprueba si el código de cuenta introducido no está en cuentas
        ...
        se ha introducido.*/
        let saldoActual = this.cuentas[codigoCad];
        ...
        this.cuentas[codigoCad] = nuevoSaldo;
    }
    ...
    eliminarCuenta(codigo) { //Método eliminarCuenta que recibe el código de una cuenta
        ...
        if(!codigoCad) {
            ...
            if(this.cuentas[codigoCad] !== 0) { //Si el saldo de la cuenta es distinto de cero, no se puede eliminar la cuenta
                console.log("Error: La cuenta tiene saldo distinto de 0.");
            }
            else { //Si el saldo de la cuenta es cero, se elimina la cuenta
                delete this.cuentas[codigoCad];
            }
        }
    }
    ...
    mostrarCuentas() { //Método mostrarCuentas
        let contenedor = document.getElementById("resultado"); //con document.getElementById accedemos al elemento del HTML con el id indicado (resultado)
        contenedor.innerHTML = `<strong>${this.nombre}</strong><br>______________<br><br>`; //Con innerHTML se puede escribir contenido HTML dentro del elemento que se indica (contenedor)
